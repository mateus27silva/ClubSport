  ...
  return (
    <div className="space-y-5 pb-24 max-w-lg mx-auto">
      ...
      <section className="px-4">
        <h2 className="text-sm font-bold text-zinc-300 mb-3 flex items-center justify-between">
          <span className="flex items-center gap-1.5">
            <Radio className="w-4 h-4 text-red-500 animate-pulse" />
            <span>Live Challenges</span>
          </span>
          <span className="text-xs text-orange-400 cursor-pointer hover:underline" onClick={() => setSelectedChallenge('all')}>
            View All
          </span>
        </h2>
        <div className="flex space-x-4 overflow-x-auto pb-2 scrollbar-none">
          {stories.map((item, index) => (
            ...
            >
              ...
              <span className="text-xs text-zinc-300 font-medium group-hover:text-orange-400 max-w-[70px] truncate text-center">
                {item.userName}
              </span>
            </div>
          ))}
      ...
      <div className="space-y-6 px-4">
        {processedActivities.length === 0 ? (
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center space-y-3">
            ...
            <h3 className="text-sm font-bold text-white">Nenhuma publicação encontrada</h3>
          ...
          processedActivities.map((act) => (
            <article
              key={act.id}
              id={`activity-${act.id}`}
              className="bg-zinc-900 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-xl"
            >
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-white flex items-center gap-1.5">
                      <span
                        onClick={() => onOpenUserProfile?.({ userId: act.userId, userName: act.userName, userAvatar: act.userAvatar })}
                        className="cursor-pointer hover:text-orange-400 hover:underline"
                      >
                        {act.userName}
                      </span>
                      <span className="text-[10px] bg-zinc-800 text-orange-400 px-1.5 py-0.2 rounded font-mono">
                        {act.sport}
                      </span>
                    </h3>
                    <div className="flex items-center gap-2 text-xs text-zinc-400">
                      ...
                      {(act.locationName || act.calculatedDistanceKm !== undefined) && (
                        <span className="flex items-center gap-1.5 text-xs font-sport italic tracking-wide font-bold text-orange-400">
                          ...
                          {act.calculatedDistanceKm !== undefined && (
                            <span className="text-zinc-400 font-mono text-[10px] not-italic ml-0.5 font-normal">
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center space-x-6 text-sm font-semibold text-zinc-300">
                <button
                  onClick={() => onToggleLike(act.id)}
                  className={`flex items-center space-x-1.5 transition-colors ${
                    act.isLiked ? 'text-orange-500' : 'hover:text-orange-400'
                  }`}
                >
                ...
                <button
                  ...
                  className="flex items-center space-x-1.5 hover:text-orange-400 transition-colors"
                >
              ...
              <p className="text-xs text-zinc-200 leading-relaxed">
                <span
                  onClick={() => onOpenUserProfile?.({ userId: act.userId, userName: act.userName, userAvatar: act.userAvatar })}
                  className="font-bold text-white mr-2 cursor-pointer hover:text-orange-400 hover:underline"
                >
                  {act.userName}
                </span>
                {act.caption}
              ...
              {activeCommentPostId === act.id && (
                <div className="pt-2 border-t border-zinc-800/80 space-y-2">
                  ...
                  <div className="max-h-36 overflow-y-auto space-y-2 text-xs">
                    {act.comments.map((cm) => (
                      <div key={cm.id} className="bg-zinc-800/40 p-2 rounded-lg">
                        <span
                          onClick={() => onOpenUserProfile?.({ userId: cm.userId, userName: cm.userName, userAvatar: cm.userAvatar })}
                          className="font-bold text-orange-400 cursor-pointer hover:underline"
                        >
                          {cm.userName}:{" "}
                        </span>
                        <span className="text-zinc-300">{cm.text}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      className="flex-1 bg-zinc-950 border border-zinc-700 text-xs text-white rounded-xl px-3 py-2 focus:outline-none focus:border-orange-500"
                    />
                    <button
