  ...
  return (
    <div className="space-y-6 pb-24 max-w-lg mx-auto px-4">
      ...
      <div className="flex items-center space-x-3 pt-2">
        ...
        <h1 className="text-xl font-black text-white tracking-tight">Edit Profile</h1>
      </div>

      {/* Change Photo Section */}
      <div className="flex flex-col items-center space-y-2">
        <div className="relative cursor-pointer group" onClick={handleTriggerUpload}>
          <div className="w-24 h-24 rounded-full border-4 border-orange-500 overflow-hidden bg-zinc-950 p-0.5 shadow-lg shadow-orange-500/10">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-2xl space-y-1">
          <label className="text-[10px] uppercase font-bold text-zinc-400 block">Full Name</label>
          <input
            type="text"
            value={fullName}
        ...
        {/* Username */}
        <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-2xl space-y-1">
          <label className="text-[10px] uppercase font-bold text-zinc-400 block">Username</label>
          <input
            type="text"
            value={username}
        ...
        {/* Bio */}
        <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-2xl space-y-1">
          <label className="text-[10px] uppercase font-bold text-zinc-400 block">Bio</label>
          <textarea
            rows={3}
            value={bio}
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-2xl space-y-1">
          <label className="text-[10px] uppercase font-bold text-zinc-400 block">Primary Sport</label>
          <select
            value={primarySport}
            onChange={(e) => setPrimarySport(e.target.value as SportType)}
            className="w-full bg-zinc-950 text-xs text-white font-semibold p-2 rounded-xl border border-zinc-800 focus:outline-none"
          >
          ...
          <div className="flex items-center space-x-2 bg-zinc-950 border border-zinc-800 p-2.5 rounded-xl text-xs text-white">
            ...
            >
              {REGION_PRESETS.map((preset) => (
                <option key={preset.id} value={preset.name} className="bg-zinc-900 text-white p-2">
                  {preset.name}
                </option>
              ))}
            </select>
