...
export const ConnectWatchModal: React.FC<ConnectWatchModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-3xl overflow-hidden shadow-2xl p-6 text-white space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <Watch className="w-6 h-6 text-orange-500" />
            <h2 className="text-base font-bold">Conectar Dispositivos & Relógios</h2>
          </div>
        ...
        <div className="space-y-3">
          {apps.map((appItem) => {
            ...
            return (
              <div
                key={appItem.name}
                className="flex items-center justify-between p-3.5 bg-zinc-950 border border-zinc-800 rounded-2xl"
              >
                <div className="flex items-center space-x-3">
                  <span className="text-xl">{appItem.icon}</span>
                  <div>
                    <h3 className="text-xs font-bold text-white">{appItem.name}</h3>
